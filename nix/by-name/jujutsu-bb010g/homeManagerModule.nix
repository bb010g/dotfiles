...
in
{
  imports = [
    ...
    (mapImportModuleTo (file: mkOverride 1100 (importTOML file)) [ "programs" "jujutsu" "settings" ] ./_jj-config-builtin.toml)
    (mapImportModuleTo importTOML [ "programs" "jujutsu" "settings" ] ./_jj-config.toml)
  ];
