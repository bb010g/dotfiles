...
let s:plug_dir = $CACHE
if s:plug_dir == ''
  let s:plug_dir = '~/.cache'
endif
let s:plug_dir .= '/dein'
let s:plug_dein_dir = substitute(
  \ fnamemodify(s:plug_dir . '/repos/github.com/Shougo/dein.vim', ':p'),
  \ '/$', '', '')
if !isdirectory(s:plug_dein_dir)
  call mkdir(s:plug_dein_dir, 'p')
  execute '!git clone https://github.com/Shougo/dein.vim' s:plug_dein_dir
endif
execute 'set runtimepath^=' . s:plug_dein_dir
set packpath=
...
if dein#load_state(s:plug_dir)
  call dein#begin(s:plug_dir)
  call dein#add(s:plug_dein_dir)
